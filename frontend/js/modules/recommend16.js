...
let currentRecommend16Type = 'am'; // 当前选择的彩种
...
async function generateRecommend16(type = 'am') {
  const resultDiv = document.getElementById('recommend16Result');
  ...
  try {
    // 调用后端API生成推荐
    const data = await API.generateRecommend16(type);

    // 检查是否有推荐结果
    if (!data.recommend16 || data.recommend16.length === 0) {
      resultDiv.innerHTML = '<div style="text-align:center;color:#f44336;padding:20px;">暂无推荐结果，请先采集开奖数据</div>';
      return;
    }

    // 渲染推荐结果
    renderRecommend16Result(data, type);

  } catch (error) {
    console.error('生成推荐16码失败:', error);
 ...
 */
function renderRecommend16Result(data, type) {
  const resultDiv = document.getElementById('recommend16Result');
  ...
  // 遍历7个位置的推荐号码
  data.recommend16.forEach((numbers, index) => {
 ...
 * @param {string} type - 彩种类型
 */
function handleRecommend16TypeChange(type) {
  currentRecommend16Type = type;

  // 更新按钮状态
  document.querySelectorAll('.recommend16-type-btn').forEach(btn => {
  ...
  // 重新生成推荐
  generateRecommend16(type);
}

/**
 * 初始化事件监听器
 */
function initRecommend16EventListeners() {
  // 彩种切换按钮
  document.querySelectorAll('.recommend16-type-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      const type = this.dataset.type;
      handleRecommend16TypeChange(type);
    });
  });
}
 ...
 * 初始化推荐16码模块
 */
function initRecommend16Module() {
  console.log('初始化推荐16码模块...');

  try {
    // 初始化事件监听器
    initRecommend16EventListeners();

    // 自动加载默认彩种的推荐
    generateRecommend16(currentRecommend16Type);

    console.log('✅ 推荐16码模块初始化成功');
  } catch (error) {
    console.error('❌ 推荐16码模块初始化失败:', error);
  }
}
...
// 导出模块初始化函数
window.initRecommend16Module = initRecommend16Module;

// 导出其他公共函数（供其他模块调用）
window.Recommend16Module = {
  generateRecommend16,
  renderRecommend16Result,
  handleRecommend16TypeChange
};

console.log('✅ 推荐16码模块已加载');

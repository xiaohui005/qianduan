 ...
 */
async function performFullAnalysis16(lotteryType, position) {
  console.log(`开始分析推荐16码命中情况 - 彩种: ${lotteryType}, 位置: ${position}`);
      ...
      const promises = batch.map(async (periodInfo) => {
        ...
        try {
          ...
          const recommendRes = await fetch(
            `${window.BACKEND_URL}/api/recommend16_by_period?lottery_type=${lotteryType}&period=${period}`
          );
...
function renderAnalysisTable16(analysisResults, position) {
    ...
    const rowClass = item.isHit ? 'hit-row' : '';
	    ...
	    html += `
	      <tr class="${rowClass}">
	        ...
	        <td class="qrcode-cell" id="qrcode-16-${item.recommendPeriod}"></td>
  ...
  `;
	  ...
	  const qrEntries = [];
	  analysisResults.forEach(item => {
	    const qrcodeElement = document.getElementById(`qrcode-16-${item.recommendPeriod}`);
...
async function loadAndAnalyze16() {
  ...
  try {
    ...
    // 获取当前位置的分析结果
    const results = await performFullAnalysis16(currentLotteryType, currentPosition);
 ...
 * 渲染当前位置的分析结果
 */
async function renderCurrentPosition16() {
  ...
  if (!results) {
    ...
    try {
      const newResults = await performFullAnalysis16(currentLotteryType, currentPosition);
      analysisCache[currentPosition] = newResults;

      // 渲染统计面板
      renderStatsPanel16(newResults);

      // 渲染表格
      renderAnalysisTable16(newResults, currentPosition);
    ...
    // 使用缓存的数据
    renderStatsPanel16(results);
    renderAnalysisTable16(results, currentPosition);
  }
}
 ...
 */
function handleTypeChange16(type) {
  currentLotteryType = type;

  // 更新按钮状态
  document.querySelectorAll('#recommend16HitAnalysisPage .type-btn').forEach(btn => {
 ...
 * @param {number} position - 位置（1-7）
 */
function handlePositionChange16(position) {
  currentPosition = position;

  // 更新按钮状态
  document.querySelectorAll('#recommend16HitAnalysisPage .tab-btn').forEach(btn => {
    btn.classList.remove('active');
...
function exportToCSV16() {
  ...
  link.setAttribute('download', `推荐16码命中分析_${currentLotteryType}_第${currentPosition}位_${new Date().toISOString().slice(0,10)}.csv`);
...
function initRecommend16HitAnalysis() {
  console.log('初始化推荐16码命中分析页面...');

  // 1. 绑定彩种切换
  document.querySelectorAll('#recommend16HitAnalysisPage .type-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      handleTypeChange16(this.dataset.type);
    });
  });
  ...
  document.querySelectorAll('#recommend16HitAnalysisPage .tab-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      handlePositionChange16(parseInt(this.dataset.position));
    });
  });
  ...
  const exportBtn = document.getElementById('export16Btn');
  if (exportBtn) {
...
// 将初始化函数暴露到全局作用域
window.initRecommend16HitAnalysis = initRecommend16HitAnalysis;
